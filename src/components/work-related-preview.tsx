...
const WorkRelatedPreview = ({ title, slug, images, youTubeId, cover, appleMusic }: Work) => {
  ...
  return (
    <Link
      as={`/works/${slug}`}
      href="/works/[slug]"
      className={`col-span-1 lg:col-span-2 flex lg:block items-center rounded-lg bg-white overflow-hidden drop-shadow-sm hover:shadow-xl transition-shadow duration-200`}
      data-aos="fade-in"
    >
      <Image
